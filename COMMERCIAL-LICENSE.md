# Commercial License — POSMA

*[Wersja polska →](COMMERCIAL-LICENSE.pl.md)*

POSMA is **dual-licensed**:

| Who you are | What applies | Cost |
|---|---|---|
| Private individual, home user, hobbyist | [PolyForm Noncommercial 1.0.0](LICENSE.md) | **Free** |
| Student, teacher, academic researcher | [PolyForm Noncommercial 1.0.0](LICENSE.md) | **Free** |
| Registered charity, public research, health, safety, environmental or government body | [PolyForm Noncommercial 1.0.0](LICENSE.md) | **Free** |
| A company, or anyone using POSMA *for* a company | **This commercial license** | **Paid** |
| A company or public/state institution with a prior arrangement | **This commercial license** | **Free — see below** |

## What counts as commercial use

Any use by or on behalf of a company — as the noncommercial license defines
"your company" — needs a commercial license. In practice that includes:

- installing POSMA on company-owned or company-managed machines,
- using POSMA as part of providing IT services to clients,
- bundling or shipping POSMA with a product or an appliance,
- using POSMA in internal company operations of any kind.

Using POSMA at home, on your own machine, for your own purposes is free —
including if you happen to be employed somewhere. What matters is who the
use is *for*.

## Free commercial licenses

The author may grant a commercial license free of charge. Companies and
public or state institutions are welcome to ask — typical cases are
evaluation before committing, schools and similar institutions, and
situations where charging simply doesn't make sense.

This is discretionary. It is not an entitlement that follows from being a
particular kind of organisation, and it does not apply retroactively: it
takes effect once agreed with the author, in advance and in writing. Absent
such an agreement, the terms in the table above apply as written.

## Getting a commercial license

Contact: **kosma.brzezawski@gmail.com** — please include the company name and
roughly how many machines POSMA would run on.

Pricing, term and the exact contract text are agreed per case; there is no
click-through commercial license yet.

## Why not simply "open source"?

POSMA is source-available rather than open source. The full source is
published so anyone can read, audit and modify it — which matters a great
deal for software that asks for administrator rights on your machine — but
companies are expected to pay for what they use commercially. This is the
same arrangement WinRAR popularised: free for the person at home, licensed
for the business.

---

> **Not legal advice.** This file summarises the licensing intent in plain
> language. The binding noncommercial terms are the verbatim PolyForm text
> in [LICENSE.md](LICENSE.md). Before selling commercial licenses or
> relying on this arrangement in a dispute, have a qualified lawyer in your
> jurisdiction review it — it was assembled by the project author, not by
> counsel.
